...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .summary { margin: 4px 0; display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; flex-shrink: 0; }
        ...
        .denda-positive { color: #dc2626; font-weight: 700; }
        .denda-zero { color: #059669; font-weight: 700; }
...
<body>
    <div class="page">
        <div class="header">
            <div class="header-row">
                ...
                <div class="meta">
                    ...
                    <div class="meta-line">Total Denda: <strong>Rp{{ number_format($totalDenda ?? 0, 0, '', '.') }}</strong></div>
                </div>
            </div>
        </div>
                    ...
                    @forelse($peminjamans as $index => $pengembalian)
                        @php
                            ...
                            if (str_contains($pengembalian->tujuan ?? '', 'Baik:')) {
                                preg_match('/Baik:(\d+), Lecet:(\d+), Rusak:(\d+), Hilang:(\d+)/', $pengembalian->tujuan, $matches);
                                if (count($matches) == 5) {
                                    $countBaik = $matches[1];
                        ...
                        <tr>
                            ...
                            <td class="text-center">{{ $pengembalian->tgl_dikembalikan ? \Carbon\Carbon::parse($pengembalian->tgl_dikembalikan)->format('d/m/y') : '-' }}</td>
                            <td class="text-center" style="font-size: 6px;">
                                @if($countBaik > 0) B:{{ $countBaik }} @endif
                                @if($countLecet > 0) L:{{ $countLecet }} @endif
                                @if($countRusak > 0) R:{{ $countRusak }} @endif
                                @if($countHilang > 0) H:{{ $countHilang }} @endif
                                @if($countBaik == 0 && $countLecet == 0 && $countRusak == 0 && $countHilang == 0) - @endif
                            </td>
                            <td class="text-center">
                                @if($totalDendaItem > 0)
                                    <span class="denda-positive">Rp{{ number_format($totalDendaItem, 0, '', '.') }}</span>
                                @else
        ...
        <div class="footer">
            <div>E-Laptop System | Laporan Pengembalian</div>
            <div>Hal. 1/1</div>
        </div>
    </div>
</body>
