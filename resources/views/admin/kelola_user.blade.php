...
@section('content')
    @if (session('success'))
        <div
            class="mb-6 flex items-center gap-3 bg-emerald-50 border border-emerald-200 text-emerald-600 px-6 py-4 rounded-2xl shadow-sm animate-fade-in">
    ...
    @if (session('error'))
        <div
            class="mb-6 flex items-center gap-3 bg-rose-50 border border-rose-200 text-rose-600 px-6 py-4 rounded-2xl shadow-sm animate-fade-in">
    ...
    <div class="space-y-6 mb-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 max-w-2xl">Atur data anggota, perbarui informasi, dan ekspor laporan PDF member
                    dengan cepat dan rapi.</p>
            </div>
            <button onclick="toggleModal('modal-tambah')"
                class="inline-flex items-center justify-center gap-2 rounded-2xl bg-cyan-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-cyan-200 transition hover:bg-cyan-700 active:scale-95">
        ...
        <div class="grid gap-4 lg:grid-cols-[1fr_auto] items-center">
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div>
                    <label class="block text-[11px] font-bold uppercase tracking-[0.32em] text-slate-500">Tanggal
                        Mulai</label>
                    <input type="date" name="tgl_mulai" form="export-pdf-user" value="{{ $startDate }}"
                        class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 outline-none focus:border-cyan-500" />
                </div>
                <div>
                    <label class="block text-[11px] font-bold uppercase tracking-[0.32em] text-slate-500">Tanggal
                        Selesai</label>
                    <input type="date" name="tgl_selesai" form="export-pdf-user" value="{{ $endDate }}"
                        class="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 outline-none focus:border-cyan-500" />
                </div>
                <div class="sm:col-span-2 xl:col-span-2 flex items-end gap-3">
                    <button type="submit" form="export-pdf-user"
                        class="w-full rounded-2xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">Export
                        PDF</button>
                    <a href="{{ route('admin.kelola_user') }}"
                        class="w-full rounded-2xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Reset</a>
                </div>
            </div>
                    ...
                    <tr class="hover:bg-emerald-50/30 transition-colors group">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs font-medium uppercase">
                                        <span
                                            class="{{ $user->is_blacklisted ? 'text-rose-600' : 'text-emerald-600' }}">{{ $user->role }}</span>
                                        @if ($user->is_blacklisted)
                                            <span
                                                class="ml-2 inline-flex items-center gap-1 rounded-full bg-rose-100 px-2 py-1 text-[10px] font-bold text-rose-700">Blacklist</span>
                                        @endif
                                    </p>
                                </div>
                            </div>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <button
                                    onclick="openEditModal('{{ $user->id }}', '{{ $user->name }}', '{{ $user->email }}', '{{ $user->no_hp }}', '{{ $user->role }}', {{ $user->is_blacklisted ? 'true' : 'false' }})"
                                ...
                                <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="w-9 h-9 flex items-center justify-center rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition-all">
        ...
        <div class="px-6 py-5 bg-gray-50/50 flex items-center justify-between">
            ...
            <div class="flex gap-2">
                <a href="{{ $users->previousPageUrl() }}"
                    class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold {{ $users->onFirstPage() ? 'text-gray-300 pointer-events-none' : 'text-gray-900' }}">Kembali</a>
                <a href="{{ $users->nextPageUrl() }}"
                    class="px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold {{ !$users->hasMorePages() ? 'text-gray-300 pointer-events-none' : 'text-gray-900' }}">Selanjutnya</a>
            </div>
        </div>
    </div>
    ...
    <div id="modal-tambah" class="fixed inset-0 z-[70] hidden items-center justify-center p-4">
        ...
        <div class="bg-white rounded-[2.5rem] w-full max-w-lg p-10 relative z-10 shadow-2xl">
            <h2 class="text-2xl font-black text-slate-900 mb-6 uppercase italic">Tambah <span
                    class="text-emerald-500">Member</span></h2>
            <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Nama Lengkap</label>
                    <input type="text" name="name" required
                        class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-emerald-500 outline-none transition-all text-sm font-semibold">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Email</label>
                        <input type="email" name="email" required
                            class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-emerald-500 outline-none text-sm font-semibold">
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" name="is_blacklisted" id="add-is_blacklisted" value="1"
                        class="rounded-lg border-slate-300 text-rose-600 focus:ring-rose-500">
                ...
                <div class="flex gap-3 pt-4">
                    <button type="button" onclick="toggleModal('modal-tambah')"
                        class="flex-1 py-4 font-black text-[10px] uppercase tracking-widest text-slate-400">Batal</button>
                    <button type="submit"
                        class="flex-[2] bg-[#062c21] text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg">Simpan
                        Member</button>
                </div>
            </form>
        </div>
    ...
    <div id="modal-edit" class="fixed inset-0 z-[70] hidden items-center justify-center p-4">
        ...
        <div class="bg-white rounded-[2.5rem] w-full max-w-lg p-10 relative z-10 shadow-2xl">
            <h2 class="text-2xl font-black text-slate-900 mb-6 uppercase italic">Edit <span
                    class="text-blue-500">Member</span></h2>
            <form id="form-edit" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="edit-name" required
                        class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:border-blue-500 outline-none text-sm font-semibold">
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Email</label>
                ...
                <div class="flex items-center gap-3">
                    <input type="hidden" name="is_blacklisted" value="0">
                    <input type="checkbox" name="is_blacklisted" id="edit-is_blacklisted" value="1"
                        class="rounded-lg border-slate-300 text-rose-600 focus:ring-rose-500">
                    <label for="edit-is_blacklisted" class="text-sm font-semibold text-slate-700">Blacklist user</label>
                </div>
                <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Password (Kosongkan
                ...
                <div class="flex gap-3 pt-4">
                    <button type="button" onclick="toggleModal('modal-edit')"
                        class="flex-1 py-4 font-black text-[10px] uppercase tracking-widest text-slate-400">Batal</button>
                    <button type="submit"
                        class="flex-[2] bg-blue-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg">Update
                        Data</button>
                </div>
            </form>
        </div>
