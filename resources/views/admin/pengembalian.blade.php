...
<div class="flex justify-between items-end mb-8">
    <div>
        ...
        <p class="text-slate-500 font-medium text-sm mt-1">Log pengembalian laptop berdasarkan kondisi dan denda.</p>
    </div>
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.pengembalian.export_pdf', request()->only(['search'])) }}" class="inline-flex items-center justify-center gap-2 rounded-xl border border-cyan-200 bg-white px-5 py-3 text-sm font-semibold text-cyan-700 shadow-sm transition hover:border-cyan-300 hover:bg-cyan-50">
            <i class="fas fa-file-pdf text-sm"></i> Export PDF
        </a>
        @if(session('success'))
            <div class="bg-cyan-100/50 border border-cyan-200 text-cyan-700 px-4 py-2 rounded-xl text-xs font-bold animate-pulse backdrop-blur-sm">
...
<form action="{{ route('admin.pengembalian') }}" method="GET" class="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-4 mb-6 hover:shadow-md transition-all duration-300">
    <div class="relative flex-1">
        <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama peminjam atau laptop..." class="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400 transition-all text-sm font-medium hover:border-slate-300">
    </div>
    <button type="submit" class="bg-gradient-to-r from-cyan-500 to-cyan-600 text-white px-8 py-3 rounded-xl font-bold text-sm hover:from-cyan-600 hover:to-cyan-700 transition-all duration-200 hover:shadow-lg hover:scale-105 active:scale-95">
        Cari Data
    </button>
</form>
    ...
    <table class="w-full text-left border-collapse">
        <thead>
            <tr class="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center w-36">Kode Pinjam</th>
                <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Laptop & Peminjam</th>
                <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Tgl Dikembalikan</th>
                <th class="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Total Denda</th>
                ...
                <tr class="hover:bg-cyan-50/40 transition-all duration-200 group">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 shrink-0 rounded-lg bg-gradient-to-br from-cyan-100 to-teal-100 flex items-center justify-center text-cyan-600 font-bold uppercase group-hover:scale-110 transition-transform duration-200">
                                {{ substr($data->alat->nama_alat ?? 'L', 0, 1) }}
                            </div>
                            <div>
                                <p class="font-bold text-slate-900 text-sm mb-0.5 leading-none">{{ $data->alat->nama_alat ?? 'Laptop Dihapus' }}</p>
                                <p class="text-[10px] text-slate-500 font-medium uppercase tracking-tight">{{ $data->user->name ?? 'User Unknown' }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex justify-center gap-2">
                            ...
                            <form action="{{ route('admin.peminjaman.destroy', $data->id) }}" method="POST" onsubmit="return confirm('Hapus permanen riwayat ini?')" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-9 h-9 flex items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-rose-50 hover:text-rose-500 transition-all duration-200 hover:scale-110 active:scale-95 border border-slate-200 hover:border-rose-200">
...
<div id="modalDetail" class="fixed inset-0 z-[100] hidden items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl transform transition-all">
        <div class="bg-gradient-to-r from-cyan-500 to-teal-500 rounded-t-2xl p-5 text-white flex justify-between items-center">
            <h3 class="font-bold text-lg flex items-center gap-2">
                <i class="fas fa-receipt"></i> Detail Pengembalian
            </h3>
            <button onclick="closeDetailModal()" class="text-white text-xl hover:scale-110 transition-transform duration-200">
        ...
        <div class="p-5 pt-3 border-t border-slate-200 bg-slate-50 rounded-b-2xl">
            <button onclick="closeDetailModal()" class="w-full bg-gradient-to-r from-slate-200 to-slate-300 py-2.5 rounded-lg text-sm font-bold text-slate-700 hover:from-slate-300 hover:to-slate-400 transition-all duration-200 hover:shadow-md">
                Tutup
            </button>
        </div>
    </div>
    ...
    function showDetailModal(data, countBaik, countLecet, countRusak, countHilang, dendaTerlambat, dendaKondisi) {
            ...
            kondisiHtml = `
                <div class="bg-gradient-to-br from-cyan-50 to-teal-50 p-4 rounded-xl border border-cyan-200">
                    <p class="font-bold text-sm text-slate-900 mb-3 flex items-center gap-2">
                        <i class="fas fa-laptop text-cyan-600"></i> Detail Kondisi per Unit:
                    </p>
        ...
        const html = `
            <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between border-b border-slate-200 pb-2">
                    <span class="text-slate-600 font-medium">Laptop:</span>
