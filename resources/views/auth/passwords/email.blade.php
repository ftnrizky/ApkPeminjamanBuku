    ...
    <div class="w-full max-w-md bg-white rounded-[2rem] p-8 md:p-10 shadow-2xl shadow-emerald-900/10 border border-gray-100 relative overflow-hidden">
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required class="w-full px-5 py-3.5 bg-gray-50 border-2 border-transpaE-Laptop rounded-2xl focus:border-emerald-500 focus:bg-white outline-none transition-all text-sm font-bold @error('email') border-rose-500 @enderror" placeholder="email@example.com">
