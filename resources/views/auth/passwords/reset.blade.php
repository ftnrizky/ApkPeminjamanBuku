    ...
    <div class="w-full max-w-md bg-white rounded-[2rem] p-8 md:p-10 shadow-2xl shadow-emerald-900/10 border border-gray-100 relative overflow-hidden">
        ...
        <form method="POST" action="{{ route('password.update') }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ $email ?? old('email') }}" required class="w-full px-5 py-3.5 bg-gray-50 border-2 border-transpaE-PUSTAKA rounded-2xl focus:border-emerald-500 focus:bg-white outline-none transition-all text-sm font-bold @error('email') border-rose-500 @enderror" readonly>
            </div>

            <div>
                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">New Password</label>
                <input type="password" name="password" required class="w-full px-5 py-3.5 bg-gray-50 border-2 border-transpaE-PUSTAKA rounded-2xl focus:border-emerald-500 focus:bg-white outline-none transition-all text-sm font-bold @error('password') border-rose-500 @enderror" placeholder="••••••••">
            ...
            <div>
                ...
                <input type="password" name="password_confirmation" required class="w-full px-5 py-3.5 bg-gray-50 border-2 border-transpaE-PUSTAKA rounded-2xl focus:border-emerald-500 focus:bg-white outline-none transition-all text-sm font-bold" placeholder="••••••••">
            </div>
