...
class PeminjamanController extends Controller
{
    ...
    public function storePeminjaman(Request $request)
    {
        ...
        // ── Catat log ────────────────────────────────────────────────────────
        ActivityLogger::adminBuatPeminjaman($peminjaman->load(['user', 'alat']));
    ...
    public function destroyPeminjaman($id)
    {
        ...
        ActivityLogger::adminHapusPeminjaman($pinjam);
    ...
    public function verifikasiPeminjaman(Request $request, $id)
    {
        ...
        if ($status == 'disetujui') {
            ...
            // ── Catat log ────────────────────────────────────────────────────
            ActivityLogger::adminSetujuiPeminjaman($pinjam, $jumlahDisetujui);

            Notifikasi::create([
                'user_id'    => $pinjam->user_id,
                'title'      => 'Peminjaman Disetujui',
                'message'    => 'Peminjaman ' . $pinjam->alat->nama_alat . ' telah disetujui dengan jumlah ' . $jumlahDisetujui . ' unit.',
                'type'       => 'success',
                'icon'       => 'fas fa-check-circle',
                'action_url' => route('peminjam.kembali'),
            ]);

            session()->flash('peminjaman_disetujui', 'Peminjaman ' . $pinjam->alat->nama_alat . ' telah disetujui! Jumlah: ' . $jumlahDisetujui . ' unit');
            ...
            // ── Catat log ────────────────────────────────────────────────────
            ActivityLogger::adminTolakPeminjaman($pinjam);

            Notifikasi::create([
                'user_id'    => $pinjam->user_id,
                'title'      => 'Peminjaman Ditolak',
                'message'    => 'Permintaan peminjaman ' . $pinjam->alat->nama_alat . ' telah ditolak.',
                'type'       => 'danger',
    ...
    public function kembalikanPeminjaman(Request $request, $id)
    {
        ...
        if (in_array($kondisi, ['baik', 'lecet', 'rusak'])) {
            $pinjam->alat()->increment('stok_tersedia', $pinjam->jumlah);
        }

        // ── Catat log ────────────────────────────────────────────────────────
        ActivityLogger::adminKembalikanAlat($pinjam, $kondisi, $total_denda);
    ...
    public function pengembalian(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->whereHas('alat', fn($qA) => $qA->where('nama_alat', 'like', "%{$search}%"))
    ...
    {
        $query = Peminjaman::with(['user', 'alat'])->whereNotNull('tgl_dikembalikan');
    ...
    public function bayarDenda(Request $request, $id)
    {
        ...
        $data->update([
            'is_denda_lunas'    => $isLunas,
            'denda_dibayar'     => $data->total_denda,
    ...
    public function verifikasiDenda($id, $aksi)
    {
        $data = Peminjaman::findOrFail($id);
